...
- (UIView *)contentViewForSection:(NSInteger)index{
	...
	if (![_totalViews objectForKey:indexNum]) {
		UIView *content = [self.dataSource zsScrollView:self contentViewForSection:index];
...
- (void)contentViewTapAction:(UITapGestureRecognizer *)tap{
	if (self.delegate) {
		[self.delegate zsScrollView:self didSelectSectionAtIndex:self.currentPageIndex];
	}
}
...
- (void)drawRect:(CGRect)rect {
	...
	if (!_totalPageCount) {
		if (self.dataSource) {
			_totalPageCount = [self.dataSource numberOfSectionsInZSScrollView:self];
			[self moveToEndRightward:NO];
			[self.animationTimer resumeTimerAfterTimeInterval:self.animationDuration];
		}else{
			_totalPageCount = 0;
