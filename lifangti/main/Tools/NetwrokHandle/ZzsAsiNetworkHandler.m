...
#import "ZzsAsiNetworkItem.h"
#import "AFNetworking.h"
@interface ZzsAsiNetworkHandler ()
<ZzsAsiNetworkDelegate>
@end;
@implementation ZzsAsiNetworkHandler

+ (ZzsAsiNetworkHandler *)sharedInstance
{
    static ZzsAsiNetworkHandler *handler = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        handler = [[ZzsAsiNetworkHandler alloc] init];
    });
    return handler;
}
    ...
    [mgr setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status)
        {
            case AFNetworkReachabilityStatusUnknown: // 未知网络
                DTLog(@"未知网络");
                [ZzsAsiNetworkHandler sharedInstance].networkError = NO;
                break;
            case AFNetworkReachabilityStatusNotReachable: // 没有网络(断网)
                [ZzsAsiNetworkHandler sharedInstance].networkError = YES;
                break;
...
{
    ZzsAsiNetworkHandler *handler = [ZzsAsiNetworkHandler sharedInstance];
    [handler.items removeAllObjects];
    handler.netWorkItem = nil;
}

- (void)netWorkWillDealloc:(ZzsAsiNetworkItem *)itme
{
